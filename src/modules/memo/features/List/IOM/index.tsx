...
const ListMemo = () => {
    ...
    const formName = 'List IOM';

    const memoRootStore = useContext(MemoRootStoreContext);
    const { getListJenisDokumen, getListKategoriDokumen, getListSubKategoriDokumen, getListDokumenIOM } = memoRootStore.memoStore;
    ...
    const [resultListDokumenIOM, setResultListDokumenIOM] = useState<ListDokumenResult[]>();
    ...
    const handleFinalFormSubmit = (values: any) => {
        ...
        if (requestKategori == 0) {
            ...
            let requestData = {
                ...
                idJenisDokumen: Number(data.jenisdokumen.value),
            ...
            const res = getListDokumenIOM(requestData).then(function (response) {
                if (response.statusCode == 200) {
                    setResultListDokumenIOM(response.result);
                }
            })
        }
                                    ...
                                    <div className="table-responsive">
                                        ...
                                        <div>
                                            ...
                                            <table id="mytableSort" className="table table-striped table-hover filterclass">
                                                ...
                                                <tbody id="myTable">
                                                    {resultListDokumenIOM?.length == 0 ?
                                                        <tr><td colSpan={8}><h2><b>Tidak Ada Data</b></h2></td></tr>

                                                        :

                                                        resultListDokumenIOM?.map(data =>
                                                            <tr>
                                                                <td className="">
                                                                ...
                                                                <td> <p><span className={data.statusDokumen == "Active" ? "label label-primary" : "label label-danger"} >{data.statusDokumen}</span></p></td>
