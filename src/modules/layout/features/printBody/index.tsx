...
const PrintBody = () => {
    ...
    const [winPrint, setWinPrint] = useState(false);
    ...
    const generatePdf = async() => {
        ...
        // MENGUKUR RATIO ZOOM PIXEL LAYAR, JIKA 100% BERARTI NORMAL SEPERTI UKURAN LAYAR AWAL
        // let hasilcurr = window.devicePixelRatio;
        // let zoompercen = (hasilcurr / initialZoomLevel) * 100
        // alert(zoompercen)
        ...
        const headerref = document.getElementById("header-id");
        setWidthHeader('850px')
        ...
        setTimeout(async ()=>{
            ...
            // html2pdf().from(tableElement).set(pdfOptions).output('dataurl').then(function(pdf){
            html2pdf().from(tableElement).set(pdfOptions).toPdf().get('pdf').then(function(pdf){
                ...
                for (var i=1; i<=totalPages; i++)
                {
                    ...
                    pdf.addImage(data, 'PNG', .4, 0.2, pdf.internal.pageSize.getWidth()-.8, 7);
    
                    // kotak yang di munculkan hanya halaman di atas satu
                    // if (i > 1){
                    //     pdf.setFillColor(255,255,255)   // warna putih
                    //     pdf.rect(.4, 0.2,  pdf.internal.pageSize.getWidth()-.8, 8,"F")  // box kotak
                    // }   
                ...
                // convert to blob to show in iframe
                let konversiBlob = pdf.output('blob');
    
                let createBlobURL = URL.createObjectURL(konversiBlob);
    
                iframeElement.src = createBlobURL;
    ...
    const onDocumentLoadSuccess = ({ numPages }) => {
        // Dapat melakukan sesuatu saat dokumen PDF berhasil dimuat
    ...
    return (
        <>
            ...
            <div ref={componentRef} className = "head-main"
                ...
                { !winPrint && 
                    (
                        <div id="header-id" ref={headerRef} style = {{visibility:'visible', width:`${widthHeader}`}}>

                            <div className='head-paper'>
                                <div className={`head-subpaper`}
                                        // style={{paddingTop: winPrint ? '30px':'0'}}
                                        >
                                        ...
                                        <div>
                                            <div className='content-body'
                                                ...
                                                <p className='content-p'
                                                    style = {{paddingTop: winPrint ? '30px':''}}>
                                                    ...
                                                    {/* <table style = {{borderCollapse:'collapse'}}>
                                                        ...
                                                        <tr style = {{border:'1px solid black'}}>
                                                            ...
                                                            <td style = {{border:'1px solid black'}}>JAMES</td>
                                                            <td style = {{border:'1px solid black'}}>A</td>
                                                        </tr>
                                                        <tr style = {{border:'1px solid black'}}>
                                                            <td style = {{border:'1px solid black'}}>A2</td>
                                                            <td style = {{border:'1px solid black'}}>ERA</td>
